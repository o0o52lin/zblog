...
Add_Filter_Plugin('Filter_Plugin_Zbp_CheckValidCode', 'LoginReg_CheckValidCode');

function LoginReg_CheckValidCode($vaidcode, $id = '')
{
    global $zbp;
...
if ($zbp->Config('LoginReg')->disable_validcode != true) {
    ...
    if (!$zbp->CheckValidCode($verifycode, 'LoginReg')) {
...
if ($zbp->Config('LoginReg')->only_one_ip) {
    $sql = $zbp->db->sql->Select($LoginReg_Table, '*', array(array('=', 'reg_IP', GetVars('REMOTE_ADDR', 'SERVER')), array('>', 'reg_Time', time() - 23 * 3600)), null, null, null);
...
$sql = $zbp->db->sql->Select($LoginReg_Table, '*', array(array('=', 'reg_InviteCode', $invitecode), array('=', 'reg_AuthorID', 0)), null, null, null);
$array = $zbp->GetListCustom($LoginReg_Table, $LoginReg_DataInfo, $sql);
$num = count($array);
if ($num == 0) {
...
if (LoginReg_CheckEmail($member->Email) == true) {
...
foreach ($GLOBALS['hooks']['Filter_Plugin_LoginReg_RegSucceed'] as $fpname => &$fpsignal) {
    $fpname($member);
}
...
$sql = $zbp->db->sql->Update($LoginReg_Table, $keyvalue, array(array('=', 'reg_ID', $reg->ID)));
$zbp->db->Update($sql);
